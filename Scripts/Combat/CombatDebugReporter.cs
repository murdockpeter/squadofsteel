...
namespace SquadOfSteelMod.Combat
{
    static class CombatDebugReporter
    {
        public static void Report(UnitGO attacker, UnitGO defender, CombatOutcome outcome)
        {
            ...
            int actualDamage = Mathf.Max(0, outcome.TargetHpBefore - outcome.TargetHpAfter);
            ...
            try
            {
                ...
                {
                    string overlaySummary = $"{attacker.unit.Name} -> {defender.unit.Name}: {(outcome.Hit ? "HIT" : "MISS")} ({Mathf.RoundToInt(outcome.HitChance * 100f)}% / roll {outcome.Roll:0.00}) dmg {actualDamage}";
                    UIManager.ShowMessage(overlaySummary, 3.5f);
                }
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[SquadOfSteel] Failed to show combat popup: {ex.Message}");
            }
        }
    }
}
